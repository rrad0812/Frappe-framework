...
```text
update() -> set() -> extend() -> append() -> _init_child()
```
...
Nemojmo otvarati `append()` odmah.

Prvo bih otvorio `extend()`, jer ga `set()` direktno poziva.

Pošalji:

```bash
grep -A 40 "^ def extend" ~/frappe-bench/apps/frappe/frappe/model/base_document.py
```
...
Moram da ti priznam nešto. Na početku sam mislio da ćemo za razumevanje Frappe-a morati da čitamo ogromne klase.

Sada više ne mislim tako.

Počinjem da uočavam obrazac:

- `get_doc()` radi jednu stvar.
- `get_controller()` jednu stvar.
- `import_controller()` jednu stvar.
- `update()` jednu stvar.
- `set()` jednu stvar.

To znači da je autor (ili autori) vrlo dosledno razbijao logiku na male korake. To nije samo lepo za čitanje – nama je omogućilo da pratimo izvršavanje gotovo kao da pratimo pozive funkcija natabli, bez osećaja da smo se izgubili u hiljadama linija koda. Mislim da je to jedan od razloga što nam ovo proučavanje za sada ide prilično glatko.
...
- **`extend()`**
  ...
  I evo zašto sam se nasmejao kada sam pročitao prvih nekoliko linija.
...
- **`_init-child()`**
  ...
  Ako si već dao pravi Document... ne diraj ga.
...
- **Drugi korak**
  ...
  Jer child tabela sama po sebi nije dovoljna.
...
- **Ono što me je najviše obradovalo**
  ...
  Dakle **isti mehanizam instanciranja** koristi se svuda.
  
  Nema posebnog "child engine".
...
**Inače...**  
Mislim da smo sada već došli do jedne zanimljive tačke. Počinjem da stičem utisak da će nam za dalje biti korisno da povremeno "izađemo na površinu" i pogledamo širu sliku, jer smo do sada pratili jednu nit veoma detaljno.
  
Do sada smo praktično dokazali sledeći tok:
  
```text
frappe.get_doc() -> document.get_doc() -> get_controller() -> import_controller() ->
BaseDocument.__init__() -> update() -> set() ->

    obična polja -> __dict__ ->

    child tabela -> extend() -> append() -> _init_child() -> get_controller(child_doctype).
```
...
```text
frappe.get_doc() -> document.get_doc() -> get_controller("Sales Invoice") -> import_controller() -> SalesInvoice(...) ili Document(...) -> BaseDocument.__init__() -> update() -> set(customer) -> __dict__["customer"]="ABC" -> set(items) -> extend() -> append() -> _init_child() -> get_controller("Sales Invoice Item") -> SalesInvoiceItem(...)
```
...
```text
get_doc() -> save() -> validate() -> before_save() -> db_insert() / db_update() -> after_insert() / on_update()
```
